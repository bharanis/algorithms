...
/* 
   Non compact tree representation.
   last level is laid out first
   previous levels are built after that
 */
print_static_mapped (bnode *root)
{
   bnode *temp;
   int d = depth(root);
   int c = count(root);
   bnode **q = malloc (sizeof (bnode *) * c  );
   int   *pos = malloc (sizeof (int) * c  );
   int qidx,pidx; 
   int i, rowcount, lvl;
   int lastprint;
   int *start_offset    = malloc (sizeof (int) * (d + 1));
   int *lvl_size        = malloc (sizeof (int) * (d + 1));
   int *internode_space = malloc (sizeof (int) * (d + 1));
   ...
   {
      start_offset[lvl]    = start_offset[lvl+1] + (node_space + internode_space[lvl+1])/2;
      lvl_size[lvl]        = ((1<<(d-1))*node_space + ((1<<(d-1))-1)*internode_space[d]) - start_offset[lvl]*2;
      if (lvl == 1) 
        internode_space[lvl] = 0;
      else 
        internode_space[lvl] = (lvl_size[lvl] - (1<<(lvl-1))*node_space)/((1<<(lvl-1)) - 1);
   }
  
   printf ("\n");
   pidx = 0;
   for (i=0; (i<d) && (pidx<c); i++) {
      printf ("%*s", start_offset[i+1] , " ");
      lastprint =  (1<<i);

      while ((pidx < c) && (pos[pidx] < (1<<(i+1)))) {
        printf ("%*s", node_space*(pos[pidx] - lastprint) + internode_space[i+1]*(pos[pidx] - lastprint), " ");
        printf ("%03d", q[pidx]->key);
        lastprint = pos[pidx];
        pidx ++;
      }
